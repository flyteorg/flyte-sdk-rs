...
| [`reusable`](reusable) | Warm containers: a pool of replicas serves many actions instead of a pod per action. One extra dependency (`union-reuse`), one changed attribute, and `reuse=` on the Python side. |
